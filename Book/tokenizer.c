...
char* brk(char* s1, int MODE, char* seps) {
	int i;
	
	for(i=0;i<strlen(s1);i++) {
		if(MODE) {
			if(sContain(s1[i],seps)) {
				return s1 + i;
			}
		}else{
...
TokenizerT *TKCreate(char *ts, int MODE, char* seps) {
	int tok_index = 0;
